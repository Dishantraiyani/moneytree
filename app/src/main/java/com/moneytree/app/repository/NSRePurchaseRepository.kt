...
object NSRePurchaseRepository {
    ...
    fun getOnlineOrderHistoryDetail(repurchaseId: String,
                              viewModelCallback: NSGenericViewModelCallback
    ) {
        apiManager.onlineOrderHistoryDetail(repurchaseId, object :
            NSRetrofitCallback<OnlineOrderInfoResponse>(viewModelCallback, NSApiErrorHandler.ERROR_REPURCHASE_INFO_DATA) {
            override fun <T> onResponse(response: Response<T>) {
                val data = response.body() as OnlineOrderInfoResponse
                if (data.status) {
                    viewModelCallback.onSuccess(response.body())
                } else {
                    errorMessageList.clear()
