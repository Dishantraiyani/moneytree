 ...
 */
class NSYoutubeViewModel(application: Application) : NSViewModel(application) {
	...
	fun getYoutubeResponse() {
		if (youtubeFullResponse?.isNotEmpty() == true) {
			youtubeResponse = Gson().fromJson(youtubeFullResponse, YoutubeResponse::class.java)
			isYoutubeVideosAvailable.value = true
		}
		if (youtubeDetail?.isNotEmpty() == true) {
			youtubeSelectedItem = Gson().fromJson(youtubeDetail, YoutubeItems::class.java)
		}
	}
