...
import com.moneytree.app.repository.network.responses.NSPendingCoinWalletListResponse
 ...
 */
class NSPendingViewModel(application: Application) : NSViewModel(application),
    NSGenericViewModelCallback {
    var redeemList: MutableList<NSPendingWalletData> = arrayListOf()
    var tempRedeemList: MutableList<NSPendingWalletData> = arrayListOf()
    var isRedeemDataAvailable = MutableLiveData<Boolean>()
    var pageIndex: String = "1"
    var redeemResponse: NSPendingCoinWalletListResponse? = null
    ...
    fun getRedeemListData(pageIndex: String, search: String, isShowProgress: Boolean, isBottomProgress: Boolean) {
        ...
        searchData = search
        NSWalletRepository.getPendingCoinWalletList(pageIndex, search, this)
    }
