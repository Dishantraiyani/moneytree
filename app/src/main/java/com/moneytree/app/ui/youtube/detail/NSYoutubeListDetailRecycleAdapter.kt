...
class NSYoutubeListDetailRecycleAdapter(
    activityNS: Activity,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val royaltyView = LayoutYoutubeDetailItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
     ...
     */
    inner class NSRoyaltyViewHolder(private val royaltyBinding: LayoutYoutubeDetailItemBinding) :
        ...
        fun bind(response: YoutubeItems) {
            with(royaltyBinding) {
                with(response) {
					if (selectedVideoId == id?.videoId) {
						clYoutube.setCardBackgroundColor(Color.parseColor(activity.resources.getString(R.string.background_gray)))
					}
