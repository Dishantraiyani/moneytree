 ...
 */
class NSStockCompleteViewModel(application: Application) : NSViewModel(application) {
    var productList: MutableList<ProductDataDTO> = arrayListOf()
	var stockListModel: MemberDetailModel? = null
	...
	var selectedStockType: String = ""

	fun checkStockList(memberId: String, isShowProgress: Boolean) {
		if (isShowProgress) {
			isProgressShowing.value = true
		}
		NSProductRepository.checkStockList(selectedStockType, memberId, object : NSGenericViewModelCallback {
			override fun <T> onSuccess(data: T) {
				isProgressShowing.value = false
				val modelData = data as NSMemberDetailResponse
				if (modelData.status) {
					stockListModel = modelData.data
				}
