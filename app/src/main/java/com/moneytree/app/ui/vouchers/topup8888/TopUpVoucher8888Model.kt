 ...
 */
class TopUpVoucher8888Model(application: Application) : NSViewModel(application) {
	...
	var downlineList: List<DownlineListItem>? = null

	/**
	 * Get register list data
	 *
	 */
	fun getTopupVoucher8888Dashboard(isShowProgress: Boolean, callback: (TopUpData?) -> Unit) {
		if (isShowProgress) {
			isProgressShowing.value = true
		}
		//{"voucher_id":"2","voucher_code":"6397104171"}
		NSVoucherRepository.getTopUpVoucher8888Dashboard(object : NSGenericViewModelCallback {
			override fun <T> onSuccess(data: T) {
				isProgressShowing.value = false
				val topUpData = data as TopUp8888Response?
				callback.invoke(topUpData?.data)
			}
